...
└── .research-yt/                   # ← HIDDEN WORKSPACE. Cache + intermediates.
    └── channels/
        └── <slug>/
...
Rules:
- The **final artifact** (the user-facing research document) goes in **cwd**, named `<topic-slug>.md` or `<topic-slug>.pdf`. Never bury it inside `.research-yt/`.
- The **workspace** (`.research-yt/`) holds everything else: channel listings, transcripts, intermediate data. It's hidden so it doesn't clutter the user's directory listing, and it's a single dotfile so it's easy to `.gitignore` or delete.
- All scripts take `--kb ./.research-yt` so the workspace path is consistent.
...
Single channel:
```bash
python3 scripts/list_channel.py --channel "@veritasium" --kb ./.research-yt --limit 50
```
...
python3 scripts/list_channel.py \
  --channel "@veritasium" \
  --channel "@3blue1brown" \
  --kb ./.research-yt --limit 50

# Equivalent:
python3 scripts/list_channel.py --channel "@veritasium,@3blue1brown" --kb ./.research-yt --limit 50
```
...
python3 scripts/fetch_transcripts.py --video "https://www.youtube.com/watch?v=XXXX" --kb ./.research-yt

# Force Whisper even when subs exist (rare — usually for non-English or low-quality auto-subs)
python3 scripts/fetch_transcripts.py --channel "@veritasium" --ids "id1" --kb ./.research-yt --force-whisper
```

### Step 5 — Synthesize the artifact

**You (Claude) do this directly** — read each transcript under `./.research-yt/channels/<slug>/videos/*/transcript.md`, extract topic-relevant passages with their `[HH:MM:SS]` anchors, and write **the final artifact to `./<topic-slug>.md` in the current working directory** (NOT inside `.research-yt/`). This is the user-facing deliverable; it lives at the top level of cwd so the user can find it immediately.
...
After the artifact is in cwd, ask the user whether to delete `./.research-yt/`. Phrase the tradeoff plainly so they can make an informed call:

> The research artifact is at `./<topic-slug>.md`. The workspace `./.research-yt/` (~<size>, transcripts + channel indexes) is no longer needed for this artifact, but keeping it makes future runs on these same channels much faster (no re-fetching metadata or re-transcribing). Delete it, or keep it for next time?
...
- Always work in the user's **current working directory** — never write workspace files inside the skill folder. The workspace is `./.research-yt/`; the final artifact is `./<topic-slug>.md` (or `.pdf`).
- Transcripts and the channel index are cached per channel inside `.research-yt/`. Re-running on the same channel(s) only fetches new videos and new metadata; adding a new channel to a multi-channel run won't re-fetch the others. Deleting `.research-yt/` discards this cache.
